...
describe('NxTransferListHalf', function() {
  ...
  it('only renders a "Remove All"/ "Transfer All" button when showMoveAll is true', function() {
    expect(quickRender({ showMoveAll: undefined }).queryByRole('button', {name: 'Remove All'})).not.toBeInTheDocument();
    expect(quickRender({ showMoveAll: null }).queryByRole('button', {name: /remove all/i})).not.toBeInTheDocument();
    expect(quickRender({ showMoveAll: false }).queryByRole('button', {name: /remove all/i})).not.toBeInTheDocument();
    expect(quickRender({ showMoveAll: true }).queryByRole('button', {name: /remove all/i})).toBeInTheDocument();
  });
...
//   it('sets the .nx-transfer-list__move-all text to "Remove All" when isSelected, otherwise "Transfer All"',
//     function() {
//     const withSelected = getShallow({ showMoveAll: true })
