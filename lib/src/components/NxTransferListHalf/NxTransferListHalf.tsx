...
import NxList from "../NxList/NxList";

export { Props, NxTransferListDataItem };

function _TransferListItem<T extends string | number = string>(
  props: TransferListItemProps<T>
) {
  const {
  ...
  const moveDownButtonTitle = moveDownDisabled
    ? "Move Down (disabled)"
    : "Move Down";
  ...
  return (
    <div role="group" aria-labelledby={displayNameId} className={classes}>
      <Tooltip {...tooltipProps}>
        <label className="nx-transfer-list__select">
          {!!onChangeProp && (
            <>
              <NxFontAwesomeIcon
                icon={checked ? faTimesCircle : faPlusCircle}
              />
              <input
                className="nx-transfer-list__checkbox"
                type="checkbox"
                checked={checked}
                onChange={onChange}
              />
          ...
          <span id={displayNameId} className="nx-transfer-list__display-name">
            {displayName}
          </span>
        </label>
      </Tooltip>
      {showReorderingButtons && (
        <NxTooltip
          title={isFilteredItem ? "Reordering is disabled when filtered" : ""}
        >
          <div className="nx-btn-bar nx-transfer-list__button-bar">
            <NxTooltip
              title={isFilteredItem ? "" : moveUpButtonTitle}
              placement="left"
            >
              <NxButton
                type="button"
                ...
                onClick={() =>
                  !moveUpDisabled && onReorderItem && onReorderItem(id, -1)
                }
              >
            ...
            <NxTooltip
              title={isFilteredItem ? "" : moveDownButtonTitle}
              placement="right"
            >
              <NxButton
                type="button"
                ...
                onClick={() =>
                  !moveDownDisabled && onReorderItem && onReorderItem(id, 1)
                }
              >
 ...
 */
export default function NxTransferListHalf<T extends string | number = string>(
  props: Props<T>
) {
  const {
      allowReordering,
    ...
    filterFn = filterFnProp
      ? partial(filterFnProp, [filterValue])
      : defaultFilterFn,
  ...
  return (
    <NxFieldset className="nx-transfer-list__half" label={label}>
      <div className="nx-transfer-list__control-box">
        <NxFilterInput
          className="nx-transfer-list__filter"
          placeholder="Filter"
        ...
        {showMoveAll && (
          <button
            type="button"
            className="nx-transfer-list__move-all"
            onClick={onMoveAllClick}
          >
            <NxFontAwesomeIcon
              icon={isSelected ? faTimesCircle : faPlusCircle}
            />
