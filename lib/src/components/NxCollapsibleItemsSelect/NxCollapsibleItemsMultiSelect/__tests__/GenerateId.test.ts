 ...
 */
import {generateId} from '../../AbstractCollapsibleItemsSelect';
...
describe('generateId', function() {
  it('generates a kebab-case id using the supplied `groupName` and `elementId`', function() {
    ...
    expect(generatedId).toEqual('nx-collapsible-items-select-testgroupname-testid');
  });
  ...
  it('generates an id even if `elementId` is `null`', function() {
    ...
    expect(generatedId).toEqual('nx-collapsible-items-select-testgroupname-null');
  });
});
