...
import NxStatefulCollapsibleItemsMultiSelect, {Props, Option} from '../NxStatefulCollapsibleItemsMultiSelect';

describe('NxStatefulCollapsibleItemsMultiSelect', function() {
  ...
  const getShallow = getShallowComponent<Props>(NxStatefulCollapsibleItemsMultiSelect, requiredProps);

  it('properly renders component using only required props', function() {
    const shallowRender = getShallow();

    expect(shallowRender).toHaveDisplayName('NxCollapsibleItemsMultiSelect');
  ...
  it('passes props to NxCollapsibleItemsMultiSelect', function() {
    const optionalProps = {
      id: 'someid',
      isOpen: true,
      disabled: true,
