...
describe('NxCombobox', function() {
    ...
    it('adds the nx-text-input--long class to the input if the `long` prop is set', function() {
        ...
        expect(rtlRender({ long: undefined }).container!.querySelector('.nx-combobox__input')).not.toHaveClass('nx-text-input--long');
        expect(rtlRender({ long: null }).container!.querySelector('.nx-combobox__input')).not.toHaveClass('nx-text-input--long');
        expect(rtlRender({ long: false }).container!.querySelector('.nx-combobox__input')).not.toHaveClass('nx-text-input--long');
        expect(rtlRender({ long: true }).container!.querySelector('.nx-combobox__input')).toHaveClass('nx-text-input--long');
    });
    ...
    it('adds the nx-text-input--short class to the input if the `short` prop is set', function() {
        ...
        expect(rtlRender({ short: undefined }).container!.querySelector('.nx-combobox__input')).not.toHaveClass('nx-text-input--short');
        expect(rtlRender({ short: null }).container!.querySelector('.nx-combobox__input')).not.toHaveClass('nx-text-input--short');
        expect(rtlRender({ short: false }).container!.querySelector('.nx-combobox__input')).not.toHaveClass('nx-text-input--short');
        expect(rtlRender({ short: true }).container!.querySelector('.nx-combobox__input')).toHaveClass('nx-text-input--short');
    });
    ...
    it('passes the disabled prop to the input and buttons', function() {
        ...
        expect(rtlRender({ disabled: undefined }).container!.querySelector('.nx-text-input__input')).toHaveProperty('disabled', false);
        expect(rtlRender({ disabled: null }).container!.querySelector('.nx-text-input__input')).toHaveProperty('disabled', false);
        expect(rtlRender({ disabled: false }).container!.querySelector('.nx-text-input__input')).toHaveProperty('disabled', false);
        expect(rtlRender({ disabled: true }).container!.querySelector('.nx-text-input__input')).toHaveProperty('disabled', true);

        expect(rtlRender({disabled: false, matches: [{ id: '1', displayName: '1' }]}).container!.querySelector('.nx-dropdown-button')).toHaveProperty('disabled', false);
        expect(rtlRender({disabled: true, matches: [{ id: '1', displayName: '1' }]}).container!.querySelector('.nx-dropdown-button')).toHaveProperty('disabled', true);
    });
    ...
    it('sets the alert role on the dropdown when it is in loading, error, or empty states', function() {
        ...
        expect(rtlRender({ loading: true }).container!.querySelector('.nx-combobox__menu')).toHaveAttribute('role', 'alert');
    ...
    it('sets aria-busy on the dropdown if loading is true', function() {
        ...
        expect(rtlRender({ loading: true }).container!.querySelector('.nx-combobox__menu')).toHaveAttribute('aria-busy', 'true');
    });
    ...
    it('does not call onSearch if focus moves into the component from an outside window while there is an error', function(){
        const onSearch = jest.fn(),
            { getByRole } = rtlRender({ searchText: 'f', error: 'err', onSearch }),
            inputElement = getByRole('combobox');
    ...
    describe('Keyboard Support', function(){
        ...
        'when the element has visual focus', async function(){
            const { getByRole, getAllByRole } = rtlRender({ matches: [{ id: '1', displayName: 'Foo' }, { id: '2', displayName: 'Boo' }] }),
            inputElement = getByRole('combobox')
        ...
        it('should open dropdown and move visual focus to the first option with aria-selected set to true' +
            ...
            const { getByRole, getAllByRole, container} = rtlRender({ matches: [{ id: '1', displayName: 'Foo' }, { id: '2', displayName: 'Boo' }] }),
                inputElement = getByRole('combobox'),
                dropdownElement = container.querySelector('.nx-combobox__menu');
        ...
        const { getByRole, container} = rtlRender({ matches: [{ id: '1', displayName: 'Foo' }, { id: '2', displayName: 'Boo' }] }),
            inputElement = getByRole('combobox'),
            dropdownElement = container.querySelector('.nx-combobox__menu');
        ...
        it('should clear input text if dropdown is close when Escape key is pressed', async function(){
            const onSearchTextChange = jest.fn(),
                { getByRole } = rtlRender({ searchText: 'a', matches: [{ id: '1', displayName: 'Foo' }, { id: '2', displayName: 'Boo' }], onSearchTextChange}),
                inputElement = getByRole('combobox');
